...
RF_model = RandomForestClassifier(n_estimators = 70, random_state = 1, max_features = 0.3, oob_score=True)
...
lgbm_params = {'learning_rate':0.2, 'boosting_type':'dart',    
              ...
              'max_depth':7,
              'num_class':9,
              'feature_faction': 0.5}  #no.of unique values in the target class not inclusive of the end value
...
#NOTE: include the below line if you are using LGBM Model and NOT for RF model
...
import pickle
filename = 'finalized_model(lgbm).sav'
